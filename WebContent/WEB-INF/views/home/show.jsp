...
<head>
<meta charset="UTF-8" />
<title>SonShop|Thông tin sản phẩm</title>
<link rel="stylesheet"
	...
	<div class="container-fluid">
		<div class="row">
			<div class="col-sm-6" style="background-color: white;">
				 <img src="./images/${product.image}" class = "KichThuoc"> 
			</div>
			<div class="col-sm-6" style="background-color: white;">
				<h2>${product.name}</h2>
				<hr>
				<h3 style="color: red">
					<fmt:formatNumber minFractionDigits="0" value="${product.price}"
						type="number" />
					₫
				</h3>
				<hr>
				<p>
					<textarea rows="12" cols="70%"
						style="border: none; font-style: italic;"> ${product.description}</textarea>
				</p>
				<hr>
				<div class="form-group">
					<c:if test="${product.amount==0}">
						<a href="home/order/${product.id}.htm" class="btn disabled"
							role="button"
							style="width: 30%; background-color: red; color: white; margin-left: 225px;">Hết hàng</a>
					</c:if>
					<c:if test="${product.amount>0}">
						<c:choose>
							<c:when test="${checkLogin==false}">
								<a href="" class="btn disabled"
									role="button"
									style="width: 30%; background-color: #358c88; color: white; margin-left: 225px;">Muangay</a>
								<p style="color: red; font-style: italic; text-align: center;">* Vui lòng đăng nhập để mua hàng</p>
							</c:when>
							<c:when test="${checkLogin==true}">
								<a href="home/order/${product.id}.htm" class="btn" role="button"
									style="width: 30%; background-color: #358c88; color: white; margin-left: 225px;">Mua ngay</a>
							</c:when>
						</c:choose>
					</c:if>
				</div>
